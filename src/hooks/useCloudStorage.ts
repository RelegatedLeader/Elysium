...
const isFirebaseAvailable = () => {
  try {
    ...
    return !!(firebaseService.auth && firebaseService.firestore && firebaseService.storage);
  } catch {
    return false;
  }
};
...
interface UseCloudStorageReturn {
  ...
  createNote: (note: Omit<CloudNote, 'id' | 'createdAt' | 'updatedAt' | 'userId'>) => Promise<void>;
...
export const useCloudStorage = (): UseCloudStorageReturn => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = firebaseService.auth.onAuthStateChange((firebaseUser) => {
  ...
  const createNote = useCallback(async (noteData: Omit<CloudNote, 'id' | 'createdAt' | 'updatedAt' | 'userId'>) => {
    if (!firebaseAvailable) {
      throw new Error('Firebase not configured. Please set up your Firebase project.');
    }
  ...
  const updateNote = useCallback(async (noteId: string, updates: Partial<CloudNote>) => {
    if (!firebaseAvailable) {
      throw new Error('Firebase not configured. Please set up your Firebase project.');
    }
    try {
  ...
  const downloadNote = useCallback(async (noteId: string, format: 'txt' | 'md' = 'md') => {
    if (!firebaseAvailable) {
      throw new Error('Firebase not configured. Please set up your Firebase project.');
    }
    try {
  ...
  const makeNotePublic = useCallback(async (noteId: string, isPublic: boolean) => {
    if (!firebaseAvailable) {
      throw new Error('Firebase not configured. Please set up your Firebase project.');
    }
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to update note visibility');
      throw err;
    }
  }, [firebaseAvailable]);
